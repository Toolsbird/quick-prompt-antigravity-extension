...
export class StorageService {
  ...
  async importData(data: { prompts?: Partial<Prompt>[]; categories?: Partial<Category>[]; skills?: Partial<Skill>[] }): Promise<{ prompts: number; skills: number; categories: number }> {
    const store = this._getStore();
    ...
    if (data.prompts) {
      for (const p of data.prompts) {
        ...
        if (p.id && store.prompts.find(ep => ep.id === p.id)) continue;
        // Skip if exact same title+content combo exists
        if (store.prompts.find(ep => ep.title === p.title && ep.content === p.content)) continue;
        // Respect free-tier limit
        if (!store.isPro && store.prompts.length >= 10) break;
    ...
    if (data.skills) {
      ...
      for (const s of data.skills) {
        ...
        if (s.id && store.skills.find(es => es.id === s.id)) continue;
        if (store.skills.find(es => es.name === s.name && es.content === s.content)) continue;
