...
exports.injectPromptToChat = injectPromptToChat;
...
class WebviewManager {
    ...
    openOrReveal() {
        ...
        this._panel = vscode.window.createWebviewPanel('quickPromptManager', 'Quick Prompt Manager', vscode.ViewColumn.One, {
            enableScripts: true,
        ...
        this._disposables.push(this._storage.onDidChange(() => this._syncWebview()));
    ...
    async _handleMessage(message) {
        switch (message.type) {
            case 'getState':
                ...
                break;
            case 'addPrompt': {
                const { title, content, category, isFavorite } = message;
                if (!title || !content) {
            ...
            case 'updatePrompt': {
                const { id, title, content, category, isFavorite } = message;
                // Ensure category exists
                const cats = this._storage.getCategories();
                if (!cats.find((c) => c.name === category)) {
                    await this._storage.addCategory(category);
